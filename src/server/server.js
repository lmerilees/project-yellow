...
app.post("/getProjectData", function(req,res){
  //NEED To FIX THIS
  pool.query("SELECT steps, tasks FROM steps INNER JOIN tasks ON steps.stepname = tasks.stepname WHERE project_name ='" + req.body.projectName +"'", function(err, result){
    if(err){
      res.send("err");
    }else{
